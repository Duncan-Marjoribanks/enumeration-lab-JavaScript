...
describe('Cinema', function () {
  ...
  it('should be able to filter films by genre', function () {
    const actual = cinema.filmSortByGenre('drama');
    assert.deepEqual(actual, [moonlight, trainspotting]);
  });

  it('should be able to check whether there are some films from a particular year', function () {
    const actual = cinema.searchFilmsByYear(2016);
  ...
  it('should be able to filter films by year', function () {
    const actual = cinema.getFilmsByYear();
    assert.deepStrictEqual(actual, { 2016:1, 2017:3, 2018:1 })
  });
